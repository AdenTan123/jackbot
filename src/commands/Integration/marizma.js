...
export default {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!process.env.MARIZMA_API_KEY) {
        return await InteractionHelper.safeEditReply(interaction, { embeds: [errorEmbed('Marizma API key not configured. Set MARIZMA_API_KEY in environment.') ] });
      }
      ...
      switch (sub) {
        ...
        case 'announce': {
          ...
          const res = await api.announce(message);
        ...
        case 'setsetting': {
          ...
          const res = await api.setSetting(payload);
        ...
        case 'banplayer': {
          ...
          const res = await api.banPlayer(userId, banned);
        ...
        case 'kick': {
          ...
          const res = await api.kickPlayer(userId, reason);
        ...
        case 'setbanner': {
          ...
          const res = await api.setBanner(banner);
